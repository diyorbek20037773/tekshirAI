...
SUBJECTS = [
    ...
    {"slug": "kimyo",       "name_uz": "Kimyo",       "icon": "🧪", "is_active": False},
    {"slug": "fizika",      "name_uz": "Fizika",      "icon": "⚡", "is_active": False},
    {"slug": "astronomiya", "name_uz": "Astronomiya", "icon": "🪐", "is_active": False},
]
...
async def seed_lessons(db: AsyncSession):
    """Idempotent seed: agar fan/mavzu mavjud bo'lmasa qo'shadi."""
    # Subjects
    subj_map = {}
    for s in SUBJECTS:
    ...
    # Biology topics
    bio = subj_map["biologiya"]
    for t in BIO_TOPICS:
        res = await db.execute(
            select(LessonTopic).where(LessonTopic.subject_id == bio.id, LessonTopic.slug == t["slug"])
        )
